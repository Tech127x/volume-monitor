...
from volume_monitor.utils.normalization import norm_device_name, normalize_name
...
class TestPackageIntegrity:
    ...
    def test_imports_work(self):
        ...
        # Test core imports
        from volume_monitor import AppKnobMonitor, CompanionTCPClient, VolumeMonitor
        from volume_monitor.audio import devices, pactl, pipewire, streams, volume_cache
        from volume_monitor.monitors import app_knobs, base, volume
...
class TestVolumeCalculations:
    ...
    @pytest.mark.parametrize(
        "output_str,expected_vol,expected_muted",
        [
...
class TestStreamAssignment:
    ...
    def test_stable_assignment(self):
        stream1: dict[str, object] = {"id": "1", "display_name": "App1", "dedupe_key": "stream:1"}
        stream2: dict[str, object] = {"id": "2", "display_name": "App2", "dedupe_key": "stream:2"}
    ...
    def test_variable_update_format(self, mock_socket_class):
        ...
        with patch.object(CompanionTCPClient, "connect", return_value=True):
            ...
            with patch.object(client, "_send") as mock_send:
                ...
                mock_send.assert_called_once_with("CUSTOM-VARIABLE test_var SET-VALUE 42\n")
