### FYI -- This program is built and tested on:
   - Bitfocus Companion BETA >= Companion: v5.1.0+9664 main-18291cbf27
   OS: Linux (v7.1.3-2-cachyos; x64)

I am very actively working on this project. If you give it a try, please send me feedback! The more I know from all of you, the better it will become!  Check frequently for updates. Thanks!

### IMPORTANT NOTE AFTER 5.1.0 UPDATE:
I have updated to run on Companion: v5.1.0+9664 main-18291cbf27.  This version has flagged shell commands as unsafe.  While volume-monitor doesn't directly execute shell commands, to change the volume, you have to be able to execute the wpctl shell command from companion therefor it must be enabled.  Here is the error message you will see if it is not enabled:

```
internal: System: Run shell command (local)
 
Disabled

Running shell commands is disabled.
This is a dangerous feature that allows running arbitrary commands on this computer, so it must be enabled explicitly. You can enable it in the Companion launcher settings, under "Dangerous Features".
```
