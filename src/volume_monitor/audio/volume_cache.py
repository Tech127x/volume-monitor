...
def save_app_volume_cache(cache: Dict[str, int]) -> None:
    ...
    try:
        APP_VOLUME_CACHE_FILE.write_text(json.dumps(cache, indent=2))
    except Exception as e:
        logger.debug(f"save app volume cache: {e}")


def app_volume_cache_key(stream: dict) -> Optional[str]:
    """Get cache key for a stream's application."""
    props = stream.get("props") or {}
    app = props.get("application.name") or stream.get("app_name") or stream.get("display_name")
    return normalize_name(app) if app else None


def get_persisted_volume_for_props(props: dict) -> Optional[int]:
